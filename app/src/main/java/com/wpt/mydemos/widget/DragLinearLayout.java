...
public class DragLinearLayout extends LinearLayout {
     ...
     * 无限制，则随意滑到顶部
     */
    private boolean isLimitBottom = false;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE://移动
                int offsetY = y - lastY;
                if (y < lastY){//向上移动时，处理移动的顶点
                    if (isLimitBottom){
                        if (getBottom() > (getScreenH(getContext()) / 2)){
//                            layout(getLeft(), getTop() + offsetY,
//                                    getRight(), getBottom()+offsetY);
                            Log.d("===wpt===","bottom=" + getBottom() + ",ScreenH=" + (getScreenH(getContext()) / 2));
                            if (offsetY + getBottom() <= (getScreenH(getContext()) / 2)){
