...
public class MyGiftView extends RelativeLayout{
    ...
    private void init() {
        drawables[0] = ContextCompat.getDrawable(getContext(), R.mipmap.ic_del_item);
        drawables[1] = ContextCompat.getDrawable(getContext(),R.mipmap.ic_album_selected);
        drawables[2] = ContextCompat.getDrawable(getContext(),R.mipmap.ic_launcher);
        drawables[3] = ContextCompat.getDrawable(getContext(),R.mipmap.ic_launcher_round);
        drawables[4] = ContextCompat.getDrawable(getContext(),R.mipmap.ic_del_item);
    ...
    /**
     * 自定义BezierEvaluator路径计算类，构造方法传入2个点，
     * 即设置为三阶贝塞尔曲线的2个控制点。依照三阶贝塞尔曲线计算公式
